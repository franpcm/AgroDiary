...
import {
  ACTIVIDAD_LABELS, CULTIVO_LABELS,
  type EntradaDiario, type Parcela, type TipoActividad, type TipoCultivo
} from '@/types';
...
export default function HistoricoPage() {
  ...
  const filteredEntries = busqueda
    ? entries.filter(e =>
        (e.descripcion + ' ' + e.resultado + ' ' + e.productos_usados + ' ' + e.notas + ' ' + e.parcela_nombre)
  ...
  const stats = {
    ...
    avgRating: filteredEntries.filter(e => e.valoracion).length > 0
      ? (filteredEntries.reduce((sum, e) => sum + (e.valoracion || 0), 0) / filteredEntries.filter(e => e.valoracion).length).toFixed(1)
      : 'N/A',
    ...
    topProducts: Object.entries(
      filteredEntries.filter(e => e.productos_usados).reduce((acc, e) => {
        acc[e.productos_usados || ''] = (acc[e.productos_usados || ''] || 0) + 1;
        return acc;
      }, {} as Record<string, number>)
    ).sort((a, b) => b[1] - a[1]).slice(0, 5),
    bestRated: filteredEntries.filter(e => e.valoracion && e.valoracion >= 4).slice(0, 5),
    worstRated: filteredEntries.filter(e => e.valoracion && e.valoracion <= 2).slice(0, 5),
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">📈 Histórico y Comparativas</h1>
          <p className="text-gray-500 mt-1">Analiza actividades pasadas y compara resultados</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setVistaComparativa(false)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
              !vistaComparativa ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            📋 Cronología
          </button>
      ...
      <div className="card p-4 mb-6">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          {(desde || hasta || parcela || tipo || busqueda) && (
            <button
              onClick={() => { setDesde(''); setHasta(''); setParcela(''); setTipo(''); setBusqueda(''); }}
      ...
      {!vistaComparativa ? (
        ...
        <>
          ...
          {Object.keys(grouped).length === 0 ? (
            <div className="card p-12 text-center text-gray-400">
              ...
              <p className="text-xl">No hay entradas en el periodo seleccionado</p>
            </div>
          ) : (
                ...
                return (
                  <div key={month}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="text-sm text-gray-400">{monthEntries.length} entradas</div>
                      <div className="flex-1 h-px bg-gray-200" />
                    </div>

                    <div className="space-y-3 ml-4 border-l-2 border-green-200 pl-6">
                      {monthEntries.map(entry => (
                        <div key={entry.id} className="card p-4 animate-fade-in relative">
                          {/* Timeline dot */}
                          <div className="absolute -left-[31px] top-5 w-4 h-4 bg-green-500 rounded-full border-2 border-white" />
                          
                          <div className="flex items-start gap-3">
                            <div className="text-2xl">
                              {ACTIVIDAD_LABELS[entry.tipo_actividad as TipoActividad]?.split(' ')[0] || '📝'}
                            </div>
                            <div className="flex-1">
                              <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
                                <span className="font-bold text-gray-700">{entry.fecha}</span>
                              ...
                              <p className="font-medium text-gray-800">
                                {ACTIVIDAD_LABELS[entry.tipo_actividad as TipoActividad] || entry.tipo_actividad}
                              </p>
                              ...
                              {entry.productos_usados && (
                                <span className="inline-block mt-1 text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded">
                                  🧪 {entry.productos_usados} {entry.dosis ? `(${entry.dosis})` : ''}
                                </span>
                              )}
                              {entry.resultado && (
                              ...
                              {entry.valoracion ? (
                                <div className="mt-1 text-yellow-500 text-sm">
                                  {'★'.repeat(entry.valoracion)}{'☆'.repeat(5 - entry.valoracion)}
                                </div>
                              ) : null}
                            </div>
                          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-yellow-50 rounded-xl p-4 text-center">
                <div className="text-3xl font-bold text-yellow-700">{stats.avgRating}</div>
          ...
          {/* Top Activities */}
          <div className="card p-6">
            <h3 className="font-bold text-lg mb-4">🏆 Actividades más frecuentes</h3>
            ...
            {stats.topProducts.length > 0 ? (
              <div className="space-y-2">
                {stats.topProducts.map(([prod, count]) => (
                  <div key={prod} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
          ...
          {/* Best Rated */}
          <div className="card p-6">
            <h3 className="font-bold text-lg mb-4">✅ Mejores resultados (4-5★)</h3>
            {stats.bestRated.length > 0 ? (
              <div className="space-y-3">
                {stats.bestRated.map(entry => (
                  <div key={entry.id} className="p-3 bg-green-50 rounded-lg">
                    <div className="text-sm font-medium text-gray-800">{entry.descripcion}</div>
                    ...
                    {entry.resultado && (
                      <p className="text-xs text-green-700 mt-1">→ {entry.resultado}</p>
                    )}
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-gray-400 text-center text-sm">Sin entradas con 4-5 estrellas</p>
            )}
          </div>

          {/* Worst Rated */}
          <div className="card p-6 lg:col-span-2">
            <h3 className="font-bold text-lg mb-4">⚠️ Resultados a mejorar (1-2★)</h3>
                ...
                {stats.worstRated.map(entry => (
                  <div key={entry.id} className="p-3 bg-red-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-xs text-gray-500">
                      ...
                      <span>{entry.parcela_nombre}</span>
                      <span className="text-yellow-500">
                        {'★'.repeat(entry.valoracion || 0)}{'☆'.repeat(5 - (entry.valoracion || 0))}
                      </span>
                    </div>
                    {entry.resultado && (
                      <p className="text-xs text-red-700 mt-1">→ {entry.resultado}</p>
                    )}
                  </div>
                ))}
              </div>
