...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isVideo && !isImage) {
      return NextResponse.json(
        { error: 'Solo se permiten imágenes y vídeos' },
        { status: 400 }
      );
    }
    ...
    const media = db.prepare('SELECT * FROM archivos_media WHERE id = ?').get(id);
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!entrada_id) {
      return NextResponse.json({ error: 'entrada_id requerido' }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const file = db.prepare('SELECT * FROM archivos_media WHERE id = ?').get(id) as { url: string } | undefined;
    if (file) {
