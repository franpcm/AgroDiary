...
export async function GET() {
  try {
    ...
    const _week = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const weekAgo = `${_week.getFullYear()}-${String(_week.getMonth() + 1).padStart(2, "0")}-${String(_week.getDate()).padStart(2, "0")}`;

    const total = db
      .prepare("SELECT COUNT(*) as count FROM entradas_diario")
      .get() as { count: number };
    const hoy = db
      .prepare("SELECT COUNT(*) as count FROM entradas_diario WHERE fecha = ?")
      .get(today) as { count: number };
    const semana = db
      .prepare("SELECT COUNT(*) as count FROM entradas_diario WHERE fecha >= ?")
      .get(weekAgo) as { count: number };
    const parcelasActivas = db
      .prepare(
        "SELECT COUNT(DISTINCT parcela_id) as count FROM entradas_diario WHERE fecha >= ?",
      )
      .get(weekAgo) as { count: number };

    const ultimaActividad = db
      .prepare(
        `
    ...
    const actividadesPorCultivo = db
      .prepare(
        `
